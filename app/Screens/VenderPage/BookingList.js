...
import { Text, View, Share, FlatList, Image, StyleSheet, Modal, TextInput, TouchableOpacity, Dimensions, DeviceEventEmitter, } from 'react-native';
...
export default class BookingList extends React.Component {
    constructor(props) {
        ...
        AppHeader({
            ...this.props.navigation, leftTitle: 'Booking List',
    ...
    getRequest(type, p) {
        ...
        Helper.globalLoader.showLoader();
        ApiCallHelper.getNetworkResponce(Constant.booking, JSON.stringify(data), Constant.APIPost).then((response) => {
            Helper.globalLoader.hideLoader();
    ...
    _renderUpComingItem = ({ item, index }) => {
        ...
        return (
            <View style={{ height: 30, marginLeft: index == 0 ? 0 : 10 }}>
                <TouchableOpacity onPress={() => { this.clickAction(item.type) }}>
                    <View style={{ flexDirection: 'row', borderWidth: 1, backgroundColor: this.state.serviceType == item.type ? 'black' : 'white', borderRadius: 20, paddingHorizontal: 15, paddingVertical: 5, alignItems: 'center' }}>
                        <Text style={{ fontFamily: fonts.PoppinsExtraBold, color: this.state.serviceType == item.type ? 'white' : 'black', fontSize: 12, fontWeight: 'bold' }}>{item.title}</Text>
                    </View>
                </TouchableOpacity>
            </View>
        )
            ...
            <View style={{ backgroundColor: 'white', flex: 1 }}>
                ...
                <View style={{ backgroundColor: 'white', paddingTop: 10, marginHorizontal: 10, paddingBottom: 10 }}>
                    <FlatList
                        ...
                        data={Helper.userData.role_id == '4' ? this.state.arrItem1 : this.state.arrItem}
                        horizontal
    ...
    renderRequest = ({ item, index }) => {
        return <View style={{ margin: 2, marginBottom: 15, paddingVertical: index == 0 ? 0 : 5, }}>

            <TouchableOpacity style={{}} onPress={() => this.props.navigation.navigate('BookingDetails', { data: item })}>
                <Card style={{ padding: 10, top: 10, width: Dimensions.get('window').width - 25, }}>
                    <View style={{ flexDirection: 'row', flex: 1 }}>
                        <Text style={{ fontWeight: 'bold', flex: 1 }}>{item.user_details?.name}</Text>
                        <TouchableOpacity onPress={() => this.providerCall(item.provider_details?.mobile_no)} style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <View style={{ flexDirection: 'row', flex: 1 }}>
                        <Text style={{ fontSize: 12, flexShrink: 1, fontWeight: 'bold', flex: 0.7 }}>Service Date & Time:</Text>
                        <Text yle={{ fontSize: 12, flex: 1 }}> {moment(item.delivery_date).format("DD-MM-YYYY")} | {moment(item.delivery_time, "HH:mm:ss").format("HH:mm A")}</Text>
                    </View>
                    ...
                    {this.state.serviceType == "4" ?
                        ...
                        (this.state.serviceType == "1") ?
                            ...
                            : (this.state.serviceType == "3" || this.state.serviceType == "2") ?
                                ...
                                <View style={{ flexDirection: 'column' }}>
                                    ...
                                    <View style={{ flexDirection: 'row' }}>
                                        ...
                                        <TouchableOpacity onPress={() => this.getAllStatus(item.booking_id)} style={{ backgroundColor: '#000', borderRadius: 10, justifyContent: 'center', alignItems: 'center' }}>
                                            <Text style={{ color: 'white', fontSize: 12, paddingHorizontal: 10 }}>Action</Text>
                                        </TouchableOpacity>
