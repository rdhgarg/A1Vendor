...
import { Text, View, ScrollView, Image, TextInput, DeviceEventEmitter, Modal, Dimensions, FlatList, Linking, StyleSheet, TouchableOpacity, SafeAreaView, Keyboard } from 'react-native';
...
export default class Home extends React.Component {
    constructor(props) {
        ...
        AppHeader({
            ...this.props.navigation, leftTitle: 'Home', borderBottomRadius: 0,
    ...
    getRequest() {
        ...
        ApiCallHelper.getNetworkResponce(Constant.homeBookings, JSON.stringify(data), Constant.APIPost).then((response) => {
    ...
    acceptRegect(sta, id) {
        ...
        //  return
        Helper.globalLoader.showLoader();
        ApiCallHelper.getNetworkResponce(Constant.bookingAccept, JSON.stringify(data), Constant.APIPost).then((response) => {
            Helper.globalLoader.hideLoader();
    ...
    renderRequest = ({ item, index }) => {
        return <View style={{ margin: 2, paddingVertical: index == 0 ? 0 : 5 }}>

            <TouchableOpacity style={{ marginBottom: 10 }} onPress={() => this.props.navigation.navigate('BookingDetails', { data: item })}>
                <Card style={{ padding: 10, top: 10, }}>
                    <View style={{ flexDirection: 'row', flex: 1 }}>
                        <Text style={{ fontWeight: 'bold', flex: 1 }}>{item.user_details?.name}</Text>


                        <TouchableOpacity onPress={() => this.providerCall(item.user_details?.mobile)} style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <View style={{ flexDirection: 'row', flex: 1, }}>
                        <Text style={{ fontSize: 12, fontWeight: 'bold', flex: 0.7 }}>Service Date:</Text>
                        <Text style={{ fontSize: 12, flex: 1, }}>{moment(item.delivery_date).format("DD-MM-YYYY")}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', flex: 1 }}>
                        <Text style={{ fontSize: 12, fontWeight: 'bold', flex: 0.7, }}>Service Time:</Text>
                        <Text style={{ fontSize: 12, flex: 1, }}>{item.delivery_time}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', flex: 1, flexShrink: 1 }}>
                        <Text style={{ fontSize: 12, fontWeight: 'bold', flex: 0.7 }}>Location:</Text>
                        <Text style={{ fontSize: 12, flex: 1 }}>{item.address_details?.address_line_1}</Text>
                    </View>

                    <View style={{ flexDirection: 'row', flex: 1, flexShrink: 1 }}>
                        <Text style={{ fontSize: 12, fontWeight: 'bold', flex: 0.7 }}>Service Request:</Text>
                        ...
                        <>
                            {item.list == "4" ?
                                <View style={{ flexDirection: 'row' }}>
                                    <TouchableOpacity onPress={() => this.getStaff(item.booking_id)} style={[renderStyle.loginView, { width: 120 }]}>
                                ...
                                (item.list == "1") ?
                                    <View style={{ flexDirection: 'row' }}>
                                        <TouchableOpacity onPress={() => this.acceptRegect(1, item.booking_id)} style={renderStyle.loginView}>
                                            <Text style={renderStyle.accept}>Accept</Text>
                                        </TouchableOpacity>

                                        <TouchableOpacity onPress={() => this.acceptRegect(0, item.booking_id)} style={renderStyle.loginView1}>
                                    ...
                                    <Text style={{ flex: 1 }}>  {item?.staff_details?.name}</Text>
                                            ...
                                            <View style={{ flexDirection: 'row', flex: 1, flexShrink: 1 }}>
                                                <Text style={{ fontSize: 12, fontWeight: 'bold', flex: 0.7 }}>Assigned:</Text>
                                                <Text style={{ fontSize: 12, flex: 1 }}>{item?.staff_details?.name}</Text>
                                            </View>
                                            <View style={{ flexDirection: 'row', alignSelf: 'flex-end' }}>

                                                <TouchableOpacity onPress={() => this.getAllStatus(item.booking_id)} style={{ backgroundColor: '#000', borderRadius: 10, justifyContent: 'center', alignItems: 'center' }}>
                                                    <Text style={{ color: 'white', fontSize: 11, paddingVertical: 5, paddingHorizontal: 10 }}>Action</Text>
                                                </TouchableOpacity>
                                                {Helper.userData.role_id == "4" ? null :
                                                    <>
                                                        <View style={{ width: 5 }}></View>
                                                        <TouchableOpacity onPress={() => this.getStaff(item.booking_id)} style={{ backgroundColor: '#000', borderRadius: 10, justifyContent: 'center', alignItems: 'center' }}>
                                                            <Text style={{ color: 'white', fontSize: 11, paddingVertical: 5, paddingHorizontal: 10 }}>Reassign</Text>
                                                        </TouchableOpacity>
                                                    </>
                                                }
                                            </View>
    ...
    selectDate(date) {
        this.setState({ showDatePicke: false, remarkDate: moment(date).format("DD-MM-YYYY") })
    }
            ...
            <SafeAreaView style={renderStyle.safe_area_view}>
                ...
                {this.state.apiResp != '0' ?
                    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps={'handled'} contentContainerStyle={{ paddingBottom: 22 }}>
                        ...
                        {this.state.apiResp == "1" || this.state.newrequest.length <= 0 && this.state.todaysbooking.length <= 0 && this.state.upcomingbooking.length <= 0 && this.state.unassignrequest.length <= 0 && this.state.completerequest?.length <= 0 ?
                            ...
                            <>
                                <View style={{ marginHorizontal: 16, }}>
                                    {this.state.newrequest.length <= 0 ? null :
                                        <View style={{ flexDirection: 'row' }}>
                                            ...
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate('BookingList', { data: '1' })}>
                                                <Text style={[renderStyle.viewAll, { marginRight: 5 }]}>View All</Text>
                                            </TouchableOpacity>
                                        </View>
                                    }
                                    <FlatList
                                ...
                                <View style={{ marginHorizontal: 16, }}>
                                    {this.state.todaysbooking.length <= 0 ? null :
                                        <View style={{ flexDirection: 'row', marginTop: 15 }}>
                                            ...
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate('BookingList', { data: '2' })}>
                                                <Text style={[renderStyle.viewAll, { marginRight: 5 }]}>View All</Text>
                                            </TouchableOpacity>
                                        </View>
                                    }
                                ...
                                <View style={{ marginHorizontal: 16, }}>
                                    {this.state.upcomingbooking.length <= 0 ? null :
                                        <View style={{ flexDirection: 'row', marginTop: 15 }}>
                                            ...
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate('BookingList', { data: '3' })}>
                                                <Text style={[renderStyle.viewAll, { marginRight: 5 }]}>View All</Text>
                                            </TouchableOpacity>
                                        </View>
                                    }
                                ...
                                <View style={{ marginHorizontal: 16, }}>
                                    {this.state.unassignrequest.length <= 0 ? null :
                                        <View style={{ flexDirection: 'row', marginTop: 15 }}>
                                            <Text style={renderStyle.titleCss}>Unassigned Booking</Text>
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate('BookingList', { data: '4' })}>
                                                <Text style={[renderStyle.viewAll, { marginRight: 5 }]}>View All</Text>
                                            </TouchableOpacity>
                                        </View>
                                    }
                                ...
                                <View style={{ marginHorizontal: 16, }}>
                                    {this.state.completerequest?.length <= 0 ? null :
                                        <View style={{ flexDirection: 'row', marginTop: 15 }}>
                                            <Text style={renderStyle.titleCss}>Completed Booking</Text>
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate('BookingList', { data: '5' })}>
                                                <Text style={[renderStyle.viewAll, { marginRight: 5 }]}>View All</Text>
                                            </TouchableOpacity>
                                        </View>
                                    }
